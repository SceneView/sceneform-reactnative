    ...
    private void refreshAnchorsIfRequired(Frame frame) {
        ...
        for (int i = 0; i < mLocationMarkers.size(); i++) {
            try {
                ...
                //Log.d("MarkerDistance", ""+markerDistance);
                float distance;
                if(markerDistance<1){
                  distance = 1;
                }
                ...
                float newScale = (1.0f/distance);
                if(newScale < 0.5f){
                  newScale = 0.5f;
                }
                Log.d("MarkerDistance", ""+newScale + " - " + distance);
                if(newScale < 0.5f){
                  newScale = 0.5f;
                }
                marker.anchorNode.setScaleModifier(newScale);//marker.getScaleModifier()
